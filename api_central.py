    ...
    try:
        with conexion() as db:
            ...
            if tipo not in {"faena", "presupuesto", "reparacion"}:
                raise HTTPException(status_code=400, detail="Esta cita es de planificación y no se convierte en un trabajo contable")
            cliente = cita["cliente"] or "Sin cliente"
            obra = cita["observaciones"] or tipo.title()
            if tipo == "faena":
                fila = db.execute(
                    "INSERT INTO faenas (cliente, obra, fecha, ubicacion, poblacion, precio, ayudantes, estado_revision) VALUES (?, ?, ?, ?, ?, 0, 'No', 'Pendiente de revisar') RETURNING *",
                    (cliente, obra, cita["fecha"], cita["ubicacion"] or "", cita["poblacion"] or ""),
                ).fetchone()
                origen = "faena"
            elif tipo == "presupuesto":
                fila = db.execute(
                    "INSERT INTO presupuestos (cliente, num_presupuesto, fecha, bruto, iva, total_iva, presupuesto_iva, efectivo, estado, presupuesto_final, estado_revision) VALUES (?, NULL, ?, 0, 0, 0, 0, 0, 'Presupuesto', 0, 'Pendiente de revisar') RETURNING *",
                    (cliente, cita["fecha"]),
                ).fetchone()
                origen = "presupuesto"
            else:
                fila = db.execute(
                    "INSERT INTO trabajos_propios (tipo, fecha_inicio, cliente, obra, ubicacion, poblacion, observaciones, importe, estado_cobro, estado_revision) VALUES ('reparacion', ?, ?, ?, ?, ?, ?, 0, 'No cobrado', 'Pendiente de revisar') RETURNING *",
        ...
        raise HTTPException(status_code=500, detail=f"No se pudo convertir la cita: {error}") from error
...
def crear_remate(datos: RemateCreate, usuario=Depends(administrador)):
    with conexion() as db:
        ...
        if DATABASE_URL:
            fila = db.execute(
                f"INSERT INTO remates ({columnas}) VALUES ({marcadores}) RETURNING *",
                tuple(valores.values()),
            ).fetchone()
        else:
...
def borrar_remate(remate_id: int, usuario=Depends(administrador)):
    with conexion() as db:
        cursor = db.execute("DELETE FROM remates WHERE id = ?", (remate_id,))
        if cursor.rowcount == 0:
            raise HTTPException(status_code=404, detail="Remate no encontrado")
    return {"eliminado": True, "id": remate_id}
